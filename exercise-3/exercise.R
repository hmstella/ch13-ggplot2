...
county_election <- left_join(counties_map, election, by = c("fips" = "combined_fips"))
...
county_election <- mutate(county_election, winner = votes_dem > votes_gop)
dem_won <- data.frame(winner = c(T, F), party = c("Democrat", "Republican"))
county_election <- left_join(county_election, dem_won, by = "winner")
